...
from sandbox.sandbox import SandboxAnalyzer
...
class MLClassifier:
    ...
    def load_models(self):
        ...
        try:
            # Load models
            with open(os.path.join(ML_MODEL_DIR, "file_classifier.pkl"), "rb") as f:
                self.file_classifier = pickle.load(f)
                
            with open(os.path.join(ML_MODEL_DIR, "behavior_model.pkl"), "rb") as f:
                self.behavior_model = pickle.load(f)
                
            with open(os.path.join(ML_MODEL_DIR, "vectorizer.pkl"), "rb") as f:
                self.vectorizer = pickle.load(f)
            
            # Load metadata
            with open(os.path.join(ML_MODEL_DIR, "metadata.json"), "r") as f:
                self.metadata = json.load(f)
                
            logger.info("ML models loaded successfully")
            print("[+] ML models loaded successfully")
            print(f"    - File classifier accuracy: {self.metadata['file_classifier']['accuracy']:.2f}")
            print(f"    - Behavior classifier accuracy: {self.metadata['behavior_classifier']['accuracy']:.2f}")
        except Exception as e:
    ...
    def model_status(self):
        ...
        return all([
            self.file_classifier is not None,
            self.behavior_model is not None,
            self.vectorizer is not None
        ])

def ml_scan(file_path, ml_classifier):
    """Perform ML-based scanning on a file"""
    if not ml_classifier.model_status():
        print("[-] ML models not loaded or not trained")
        return False
    ...
    try:
        ...
        # File classification
        file_result = ml_classifier.classify_file(file_path)
        if file_result:
            prediction, proba = file_result
            print(f"    - File classification: {'Malicious' if prediction == 1 else 'Benign'}")
            print(f"    - Confidence: {proba:.2%}")
            
            if prediction == 1 and proba > 0.7:
                print("[!] ML detection: File classified as malicious")
                return True
        
        # Behavior prediction (simulated)
        print("[*] Simulating behavior analysis...")
        simulated_behaviors = [
            "created files",
            "accessed network",
            "modified registry"
        ]
        behavior_report = " ".join(simulated_behaviors)
        
        behavior_result = ml_classifier.classify_behavior(behavior_report)
        if behavior_result:
            prediction, proba = behavior_result
            print(f"    - Behavior analysis: {'Suspicious' if prediction == 1 else 'Normal'}")
            print(f"    - Confidence: {proba:.2%}")
            
            if prediction == 1 and proba > 0.7:
                print("[!] ML detection: Suspicious behavior detected")
                print(f"    - Detected behaviors: {', '.join(simulated_behaviors)}")
                return True
                
        return False
        
    except Exception as e:
        logger.error(f"ML scan error: {str(e)}")
        print(f"[!] Error during ML scan: {str(e)}")
        return False
